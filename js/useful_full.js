// JavaScript Document
	...
	function getIso2(input, i) {
		...
		var c = countryData.filter(function(d) { return d[accessor].toLowerCase() == input.toLowerCase(); });
	...
	function getIso3(input, i) {
		...
		if(input.toLowerCase() == 'virgin islands us' || input.toLowerCase() == 'virgin islands, u.s.') input = 'U.S. Virgin Islands';
		if(input.toLowerCase() == 'n mariana is') input = 'Northern Mariana Islands';
		if(input.toLowerCase() == 'virgin islands british'|| input.toLowerCase() == 'virgin islands, british') input = 'British Virgin Islands';
		...
		if(input == "Åland Islands" ){ input = 'Aland'; }
		...
		if(input.toLowerCase()=="bolivia, plurinational state of" ){ input="Bolivia"; }
		if(input.toLowerCase()=="brunei darussalam" ){ input="Brunei"; }
		if(input.toLowerCase()=="côte d'ivoire"){ input="Cote d'Ivoire"; }
		if(input.toLowerCase()=="congo, the democratic republic of the"){ input="Congo"; }
		if(input.toLowerCase()=="falkland islands (malvinas)"){ input="Falkland Islands (Islas Malvinas)"; }
		if(input.toLowerCase()=="french guyana"){ input="French Guiana"; }
		if(input.toLowerCase()=="micronesia, federated states of"){ input="Micronesia"; }
		if(input.toLowerCase()=="cocos islands"){ input="Cocos (Keeling) Islands"; }
		if(input.toLowerCase()=="iran, islamic republic of"){ input="Iran"; }
		if(input.toLowerCase()=="siachen glacier") return;
		if(input.toLowerCase()=="macao"){ input="Macau"; }
		if(input.toLowerCase()=="saint martin (french part)"){ input="Saint Martin"; }
		if(input.toLowerCase()=="moldova, republic of"){ input="Moldova"; }
		if(input.toLowerCase()=="macedonia, the former yugoslav republic of"){ input="Macedonia"; }
		...
		if(input.toLowerCase()=="taiwan, province of china") { input="Taiwan"; }
		if(input.toLowerCase()=="tanzania, united republic of") { input="Tanzania"; }
		if(input.toLowerCase()=="venezuela, bolivarian republic of") { input="Venezuela"; }
		...
		if (c.length == 0){
			...
			c = countryData.filter(function(d) { return d[accessor].toLowerCase() == input.toLowerCase(); });
		};
	...
	function getGeoRegion_usingISO3(input, i) {
		...
		var c = countryData.filter(function(d) { return d[accessor].toLowerCase() == input.toLowerCase(); });
		if (c.length == 0 || c == '' ){ return 'undefined'; }
		...
		text.each(function() {
			...
			while (word = words.pop()) {
				...
				if (tspan.node().getComputedTextLength() > content_width) {
					...
					tspan.text(line.join(" "));
					line = [word];
					tspan = text.append("tspan").attr("x", ttmargin).attr("y", y).attr("dy", ++lineNumber * lineHeight + dy).text(word);
...
vis = {};
	...
	function splitToLocationsSent(data, countryFrom){
		...
		data.forEach(function(d,i){
			...
			if( destinationCountries.indexOf(destinationCountry)==-1 && destinationCountry!='' && destinationCountry!=countryFrom){
